...
4. Add an entry to `manifest.json` under a mood key matching the `audio_mood` values the
   idea generator produces (freeform strings are fine — mood lookup is case-insensitive,
   and `pick_music` falls back to any track if no mood match exists), e.g.:
...
**Until this folder has tracks, the pipeline cannot render a video** — `pick_music`
raises `NoMusicAvailableError` on an empty manifest.
